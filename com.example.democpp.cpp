...
#define CQAPPID "com.example.democpp" //请修改AppID，规则见 http://d.cqp.me/Pro/开发/基础信息
...
CQEVENT int32_t _eventPrivateMsg(int32_t subType, int32_t sendTime, int64_t fromQQ, const char *msg, int32_t font)
{
	...
	//如果要回复消息，请调用酷Q方法发送，并且这里 return EVENT_INTERCEPT - 截断本条消息，不再继续处理  注意：应用优先级设置为"最高"(10000)时，不得使用本返回值
